...
_BLOCKED: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"rm\s+.*-[a-zA-Z]*r[a-zA-Z]*f.*(/boot|/etc|/usr|/bin|/sbin|/lib)\b"),
    ...
    # Wiping commands
    (re.compile(r"\b(wipefs|blkdiscard)\b"),
     "Disk wipe command"),
    ...
    (re.compile(r"(rm|cat /dev/null\s*>)\s+~/?\.(bash_history|zsh_history|sh_history)"),
...
_WARNING: list[tuple[re.Pattern, str]] = [
    ...
    # Package removal
    (re.compile(r"\b(apt|dnf|yum|pacman|zypper|apk)\b.+\b(remove|purge|autoremove|erase|uninstall)\b"),
    ...
    (re.compile(r"(>|tee)\s+/etc/"),
     "Writing to /etc — modifies system configuration"),
    ...
    (re.compile(r"sudo\s+(su|bash|sh|zsh|-s)\b"),
     "Opening a root shell session"),
    ...
    (re.compile(r"systemctl\s+(disable|mask|stop)\s+(ssh|sshd|networking|network|systemd-resolved)\b"),
